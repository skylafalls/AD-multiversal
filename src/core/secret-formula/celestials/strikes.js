...
export const pelleStrikes = {
  infinity: {
    ...
    rewardDescription: () => `Unlock ${wordCycle(PelleRifts.vacuum.name)}
  ...
  powerGalaxies: {
    ...
    rewardDescription: () => `Unlock ${wordCycle(PelleRifts.decay.name)}`,
  ...
  eternity: {
    ...
    rewardDescription: () => `Unlock ${wordCycle(PelleRifts.chaos.name)}`,
    rift: () => PelleRifts.chaos,
  },
  ECs: {
    id: 4,
    requirementDescription: () => `Reach ${formatInt(115)} TT`,
    penaltyDescription: () => `In Eternity Challenges, ${wordCycle(PelleRifts.vacuum.name)}
      IP multiplier is only ${formatPercents(0.3)} as strong and capped at ${formatPercents(0.15)}
      of the goal`,
    rewardDescription: () => `Unlock ${wordCycle(PelleRifts.recursion.name)}`,
  ...
  dilation: {
    ...
    rewardDescription: () => `Keep the Time Dilation study across Armageddon, boost Remnant gain, and unlock
      ${wordCycle(PelleRifts.paradox.name)}`,
