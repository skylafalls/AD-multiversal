...
const LAYER_DOWN = 15.954242509439325;
...
const critical_headers = [2, Math.E, 3, 4, 5, 6, 7, 8, 9, 10] as const;
...
export class Decimal {
   ...
   */
  public static readonly dNegInf = Decimal.fromComponents_noNormalize(-1, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
  /**
   ...
   */
  public static readonly dLayerSafeMax = Decimal.fromComponents(1, Number.MAX_SAFE_INTEGER, EXP_LIMIT - 1);
  /**
   * Represents the smallest Decimal where adding 1 to the layer is a safe operation. Approximately 1 / (10^^(9.007 * 10^15)).
   */
  public static readonly dLayerSafeMin = Decimal.fromComponents(1, Number.MAX_SAFE_INTEGER, -(EXP_LIMIT - 1));
  /**
   * Represents the largest finite value a Decimal can represent. Approximately 10^^(1.79 * 10^308).
   */
  public static readonly dLayerMax = Decimal.fromComponents(1, Number.MAX_VALUE, EXP_LIMIT - 1);
  /**
   * Represents the smallest non-zero value a Decimal can represent. Approximately 1 / (10^^(1.79 * 10^308)).
   */
  public static readonly dLayerMin = Decimal.fromComponents(1, Number.MAX_VALUE, -(EXP_LIMIT - 1));
  ...
  /**
   * Returns the rough equivalent for the exponent of 'decimal'.
   */
  public static expEquiv(decimal: Decimal, floor = false): Decimal {
    if (decimal.abs().lte(1)) return new Decimal(0);
    if (!floor) return decimal.max(1).log10();
    return decimal.max(1).log10().floor();
  }

  public static maxReducer(a: Decimal, b: Decimal): Decimal {
    return Decimal.max(a, b);
  };

  public static sumReducer(accumulator: Decimal, previous: Decimal): Decimal {
    return Decimal.add(accumulator, previous);
  };

  public static prodReducer(accumulator: Decimal, previous: Decimal): Decimal {
  ...
  public valueOf(): never {
    throw new TypeError("Implict coherecion from Decimal to number");
  }
  ...
  public floor(): Decimal {
    if (this.mag < 0) {
      if (this.sign === -1) return Decimal.fromComponents_noNormalize(-1, 0, 1);
      else return Decimal.fromComponents_noNormalize(0, 0, 0);
    }
  ...
  public ceil(): Decimal {
    if (this.mag < 0) {
      if (this.sign === 1) return Decimal.fromComponents_noNormalize(1, 0, 1); //The ceiling function called on something tiny like 10^10^-100 should return 1, since 10^10^-100 is still greater than 0
      else return Decimal.fromComponents_noNormalize(0, 0, 0);
    }
  ...
  public mod(value: DecimalSource, floored : boolean = false): Decimal {
    ...
    if (this.eq(Decimal.dZero) || decimal.eq(Decimal.dZero)) return Decimal.fromComponents_noNormalize(0, 0, 0);
    if (floored) {
  ...
  /**
   * Returns 'this', unless the exponent of 'this' is greater than 'max', in which case '10^max' is returned.
   */
  public clampMaxExponent(maxExp: number): Decimal {
  ...
  public static increasingInverse(
    ...
    return function(value : DecimalSource) : Decimal {
      ...
      if (maxX.lt(0)) currentCheck = false;
        ...
        if (valCheck.eq(value)) return Decimal.fromComponents_noNormalize(0, 0, 0);
  ...
  public penta_log(base: DecimalSource = 10, iterations = 100, linear = false) {
    ...
    if (this.eq(1)) return Decimal.fromComponents_noNormalize(0, 0, 0);
  ...
  /**
   * Copies the value of one Decimal instance into this Decimal instance.
   *
   * Note: This mutates the value it's called upon.
   * @param {Decimal} decimal The Decimal value to copy from.
   */
  public copyFrom(decimal: Decimal): void {
    this.mag = decimal.mag;
    this.layer = decimal.layer;
    this.sign = decimal.sign;
  }

  public plusEffectOf(effectSource: Effect): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    effectSource.applyEffect((v: DecimalSource) => result = result.plus(v));
    return result;
  };

  public plusEffectsOf(...effectSources: Effect[]): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
  ...
  public minusEffectOf(effectSource: Effect): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    effectSource.applyEffect((v: DecimalSource) => result = result.minus(v));
    return result;
  };

  public minusEffectsOf(...effectSources: Effect[]): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    for (const effectSource of effectSources) {
      if (effectSource !== null && effectSource !== undefined) {
        effectSource.applyEffect((v: DecimalSource) => result = result.minus(v));
  ...
  public timesEffectOf(effectSource: Effect): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    effectSource.applyEffect((v: DecimalSource) => result = result.times(v));
    return result;
  };

  public timesEffectsOf(...effectSources: Effect[]): Decimal {
  ...
  public dividedByEffectOf(effectSource: Effect): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    effectSource.applyEffect((v: DecimalSource) => result = result.dividedBy(v));
    return result;
  };

  public dividedByEffectsOf(...effectSources: Effect[]): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    for (const effectSource of effectSources) {
      if (effectSource !== null && effectSource !== undefined) {
        effectSource.applyEffect((v: DecimalSource) => result = result.dividedBy(v));
  ...
  public powEffectOf(effectSource: Effect): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    effectSource.applyEffect((v: DecimalSource) => result = result.pow(v));
    return result;
  };

  public powEffectsOf(...effectSources: Effect[]): Decimal {
    let result = Decimal.fromValue_noAlloc(this);
    for (const effectSource of effectSources) {
      if (effectSource !== null && effectSource !== undefined) {
        effectSource.applyEffect((v: DecimalSource) => result = result.pow(v));
